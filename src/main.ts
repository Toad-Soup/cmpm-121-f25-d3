...
const playerPosition = leaflet.latLng(0, 0); //gonna be removed by playerNav which usees GPS
...
//move the player nav higher so it can be used by other functs
...
function loadGameState() {
  ...
  if (lat && lng) {
    const pos = leaflet.latLng(Number(lat), Number(lng));
    playerNav.position = pos;
...
function distance_to_player(i: number, j: number) {
  const pp = pointCoordToIndex(playerNav.position);
  const dx = i - pp.x;
  const dy = j - pp.y;
...
function generateCells() {
  ...
  const center = pointCoordToIndex(playerNav.position);
